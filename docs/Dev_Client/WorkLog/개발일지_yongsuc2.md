...
### 완료 작업
- **재능 등급/서브등급/보상 기획서 정리 + Unity 동기화** (Y-73)
  - 원본 TS(copybarago)와 Unity C# 구현 비교 분석 — Domain 로직/JSON/테스트 1:1 일치 확인
  - `docs/02_캐릭터성장시스템.md` 기획서 정리:
    - 서브등급 개수 오류 수정: 수련생(5단)→(2단), 모험가(15단)→(5단), 정예(30단)→(10단), 달인(50단)→(17단), 전사(100단)→(33단)
    - 등급 구조 테이블 추가 (서브등급 수, 총 레벨, 시작/종료 레벨)
    - 서브 등급 진급 메카닉 상세화
    - 서브 등급 전환 보너스 2종(메인 등급/서브 등급) 구분 기술
