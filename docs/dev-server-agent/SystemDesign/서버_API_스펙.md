...
비즈니스 에러(재화 부족, 조건 미충족 등) 시 모든 API가 HTTP 400을 반환합니다. 응답 body에 `ApiResponse<T>` JSON이 포함되며, `errorCode` 필드로 에러 유형을 식별할 수 있습니다.
...
| 404 | 리소스 없음 |
| 500 | 서버 내부 오류 (`ApiResponse.Fail("INTERNAL_ERROR", ...)` 형태) |
| 503 | 서비스 상태 이상 (health check degraded) |

### 공통 응답 헤더
| 헤더 | 설명 |
|------|------|
| `X-Correlation-Id` | 요청 추적 ID (클라이언트가 보내면 그대로 반환, 없으면 서버에서 자동 생성) |
...
|--------|----------|------|------|
| GET/HEAD | `/` | X | 기본 Health Check (200 OK) |
| GET | `/health` | X | 상세 Health Check (DB/Redis 연결 상태) |
...
| 2026-03-07 | BUG-010: RewardData 구조를 클라이언트 래퍼 구조로 통일, Amount를 int로 변경 |
| 2026-03-07 | BUG-008: 모든 컨트롤러 비즈니스 에러 시 HTTP 400 반환으로 통일 |
| 2026-03-07 | 모니터링 개선: GET /health 상세 health check 추가, X-Correlation-Id 응답 헤더, 500 에러 구조화된 응답 |
| 2026-03-08 | BUG-017: EquipmentDeltaData에 WeaponSubType 필드 추가, EquipmentEntry에 Slot/WeaponSubType 컬럼 추가 |
