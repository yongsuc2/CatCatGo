...
### 가차→상점 화면 전환 + 4탭 구조 (K-21)
- 하단 NavBar "가차" → "상점"으로 변경
- ScreenType.Gacha → ScreenType.Shop 리네이밍
- GachaScreen 삭제, ShopScreen 신규 생성
- 하단 4탭 구조: 장비상점/펫상점/패키지상점/충전 (기본 탭: 장비상점)
- 각 탭 진입 시 스크롤 가능한 상품 카드 목록 표시
- 장비상점: S급 장비뽑기(보석), 장비뽑기(보석), 모험가 상자(은열쇠), 영웅상자(금열쇠)
- 펫상점: 우수 펫뽑기(보석), 일반 펫 뽑기(펫 알)
- 패키지상점/충전: 플레이스홀더
- 보석 기반 뽑기는 기존 가차 시스템(Game.PullGacha) 연동
- MainScreen 메뉴 카드 "뽑기" → "상점" 변경
- 뽑기화면_기획서.md → 상점화면_기획서.md 문서 갱신

### 합성 탭 UI 재설계 (K-22)
- 합성 탭 전면 재설계: 그룹 리스트 → 개별 장비 선택 방식
- 상단: 합성 결과 미리보기 (등급 색상 배경 + 장비 스프라이트 + 예상 스탯)
- 중단: 별(Star) 슬롯 + 재료 슬롯 (선택 장비 아이콘 + 필요 재료 표시)
- 하단: 인벤토리 그리드 (S등급 제외 전체 장비, 5열)
- 장비 클릭 → 별 슬롯 배치 → 동일 슬롯/등급 재료 자동 매칭 → 결과 미리보기
- 합성/일괄 합성 버튼 분리
- 장비화면_기획서.md 합성 탭 섹션 갱신

### 설정 팝업에 디버그 화면 버튼 추가
- SettingsPopup에 "디버그 화면" 버튼 추가
- 메뉴 → 설정 → 디버그 화면으로 접근 가능

### 보관함 헤더에 합성 아이콘 추가
- 장비 탭 보관함 헤더 우측에 합성 아이콘 버튼 배치
- 클릭 시 합성 탭으로 전환
...
### 홈 화면 메뉴 버튼 + 팝업 시스템 (K-18)
- 홈 화면 좌측 상단에 ☰(햄버거) 메뉴 버튼 추가
- MenuListPopup: 가방/우편/퀘스트/출석체크/설정 5개 항목
- InventoryPopup: 전 소모성 아이템 13종 수량 표시 (골드, 보석, 스태미나, 도전토큰, 곡괭이, 장비석, 파워스톤, 펫알, 펫사료, 유산서적 4종)
- MailPopup: 빈 우편함 안내
- SettingsPopup: 배경음/효과음 ON/OFF 토글 (PlayerPrefs 저장)
...
### 장비 화면 재진입 시 전체 필터 초기화 (K-20)
- EquipmentScreen.OnScreenEnter()에서 OnTabChanged(0) + OnFilterChanged(0) 호출
- 다른 화면 갔다가 돌아올 때 항상 장비탭 + 전체필터로 초기화
