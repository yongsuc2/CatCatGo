...
## 2026-03-08 기획서 문서 규칙 준수 검토

### 검토 범위
`docs/planning-agent/SystemDesign/` 아래 전체 기획서 (시스템 기획서 14종 + 화면 기획서 7종)

### 검토 항목 및 결과

#### 1. 수치값 포함 금지 위반
- **06_가챠시스템.md**: 가중치, 확률(%), 비용(보석/열쇠 수량), 천장 횟수, S등급 확률 등 밸런스 수치가 기획서에 직접 명시되어 있었음
  - 보석 장비뽑기: 등급별 가중치/확률 테이블, 비용(150/1350), 천장(180회), S확률(2%)
  - 모험가 상자: 가중치/확률 테이블(3:1), 비용(은열쇠 1개)
  - 영웅상자: 가중치/확률 테이블(9:3:1), 비용(금열쇠 1개)
  - 펫뽑기: 비용(298보석), 보상 수량(1~5/0~2)
  - JSON 구조 예시에 구체적 수치(150, 180, 0.02, 298, 1)
- **05_스테이지던전시스템.md**: 고블린 광부 "30개 달성" 수치
- 모두 `gacha.data.json` 참조로 교체 완료

#### 2. 기획의도 명시 — 양호
- 각 시스템의 역할, 목적, 유저 경험이 적절히 기술되어 있음

#### 3. 개념/구조 중심 — 양호 (수치 제거 후)
- 수치 제거 후 개념과 구조만 남은 상태

#### 4. 문서 간 정합성 — 참고 사항 1건
- 던전 명칭이 역기획서 용어(`00_게임개요.md`: 드래곤 둥지, 아스트랄 나무, 환검의 하늘섬)와 자체 기획 용어(`05_스테이지던전시스템.md`: 거대 벌집, 수천년 고목, 호랑이 절벽)로 이원화. 의도적 변경일 가능성이 있으나, `00_게임개요.md`의 역기획서 용어를 자체 기획 용어로 통일할지 검토 필요

### 수정 파일
- `docs/planning-agent/SystemDesign/06_가챠시스템.md` — 밸런스 수치 제거, 데이터 테이블 참조로 교체
- `docs/planning-agent/SystemDesign/05_스테이지던전시스템.md` — "30개 달성" → "목표 달성"
